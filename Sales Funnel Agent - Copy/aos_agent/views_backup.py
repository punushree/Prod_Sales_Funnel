...
from .tasks import run_post_call_analysis
...
def bolna_initiate(request):
    ...
    if not org.bolna_agent_id:
        return JsonResponse({"success": False, "error": "No Bolna agent configured for this org"}, status=400)
...
def bolna_webhook(request):
    ...
    try:
        ...
        call.save(update_fields=["duration_seconds", "status", "ended_at"])
        ...
        lead_name      = (
            ...
            or data.get("variables", {}).get("customer_name", "Customer")
        )
...
def _safe_ma_create(**kwargs):
    """
    Create ManualAnalysis safely — drops routing_type / call kwargs
    if migration 0020 has not yet been applied.
    routing_type is always mirrored into extra_data for queryability.
    """
...
def customer_list_view(request):
    ...
    try:
        ...
        all_svc = list(base_qs.filter(call_type="service").order_by("-created_at")[:60])
        mixed_qs = [m for m in all_svc
                    if (m.extra_data or {}).get("routing_type") == "service_then_sales"][:20]
    mixed_history = []
    for m in mixed_qs:
        ed = m.extra_data or {}
...
def allowed_numbers_list(request):
    ...
    if org:
        quota = org.number_quota
    else:
        fb = Organisation.objects.filter(is_active=True, is_approved=True).first()
        quota = fb.number_quota if fb else 0
...
def allowed_numbers_add(request):
    ...
    # 3. Last resort — first active approved org
    if org is None:
...
def service_call_result(request, bolna_call_id):
    ...
    _rt = (getattr(analysis, "routing_type", None)
           or ed.get("routing_type")
           or analysis.call_type
           or "service_only")
    dashboard_data = {
        "routing_type":       _rt,
...
#  Polled every 3s by the frontend modal after a service call starts.
#
#  Stage 1 → call in progress (CallLog not completed yet)
#  Stage 2 → call ended — waiting for transcript to be saved
#  Stage 3 → transcript saved — Feedback Agent running
#  Stage 4 → ManualAnalysis saved — full KPI payload returned
# ════════════════════════════════════════════════════════════════════

@csrf_exempt
def api_call_analysis_status(request, bolna_call_id):
    """
    GET /api/call/<bolna_call_id>/analysis-status/
    Returns {ready, stage, stage_label} and full KPI data when ready=True.
    """
    # ── Stage 1: find call ──────────────────────────────────────────
    ...
    if not call:
        return JsonResponse({"ready": False, "stage": 1,
                             "stage_label": "Locating call record…"})

    if call.status not in ("completed", "failed"):
        return JsonResponse({"ready": False, "stage": 1,
                             "stage_label": "Conversation in progress…"})

    # ── Stage 2: check transcript in DB ────────────────────────────
    transcript_text = ""
    try:
        transcript_text = call.transcript.full_text or ""
    ...
    if not transcript_text and turns:
        transcript_text = "\n".join(
            f"{t.get('speaker','?').upper()}: {t.get('text','')}" for t in turns
        )

    if not transcript_text:
        return JsonResponse({"ready": False, "stage": 2,
                             "stage_label": "Call ended — saving transcript to database…"})

    snippet = transcript_text[:300] + ("…" if len(transcript_text) > 300 else "")

    # ── Stage 3: check Feedback Agent ran ──────────────────────────
    analysis = None
    try:
    ...
    if not analysis:
        return JsonResponse({"ready": False, "stage": 3,
                             "stage_label": "Transcript saved — Feedback Agent analysing…",
    ...
    f2         = ed.get("fit", 0)
    f3         = ed.get("conversion", 0)
    lead_score = ed.get("lead_score", analysis.lead_score or 0)
    if not lead_score and (f1 or f2 or f3):
        lead_score = round(f1 * 0.4 + f2 * 0.35 + f3 * 0.25)
    ...
    return JsonResponse({
        ...
        "story": "", "intents": [], "signals": ed.get("buying_signals", []),
...
def call_service_agent(request):
    ...
    try:
        ...
        # ── Resolve agent ID (org → settings — NO hardcoded fallback) ─
        agent_id = (
            org.service_bolna_agent_id
            or org.bolna_agent_id
            or getattr(dj_settings, "BOLNA_SERVICE_AGENT_ID", "")
        ).strip()
        ...
        if not agent_id:
            return JsonResponse({
                "success": False,
                "error": (
                    "Service Bolna Agent ID not configured. "
                    "Set it in Django Admin → Organisations → your org → 'Service Bolna Agent Id', "
                    "or add BOLNA_SERVICE_AGENT_ID=<your-agent-id> to your .env file."
                )
            }, status=500)

        logger.info("[SERVICE CALL] org=%s agent_id=%s phone=%s", org.name, agent_id, phone)

        # ── Fire call via bolna_service helper ────────────────────
        result = initiate_call(
            agent_id   = agent_id,
            phone      = phone,
            name       = name,
        ...
        # ── Create CallLog ────────────────────────────────────────
        _safe_kwargs = {"org": org, "call_type": "service", "direction": "outbound",
                        "bolna_call_id": bolna_call_id, "started_at": timezone.now(), "status": "in_progress"}
        try:
            _safe_kwargs["routing_type"] = "service_only"
            CallLog.objects.create(**_safe_kwargs)
        except Exception:
            _safe_kwargs.pop("routing_type", None)
            CallLog.objects.create(**_safe_kwargs)
